...
const storyData = {
    ...
    // Outros blocos...
    'c1_b5_s1':{isDiagnosis:!0,chapter:1,block:'b5',title:"Integridade Estrutural (1/2): Lei de Fourier",text:"Log da I.A.: Sensores indicam uma perda de calor anormal em uma seção do casco. A energia está se propagando por condução através de painéis que deveriam ser isolantes. É preciso entender a Lei de Fourier para quantificar e mitigar essa perda.",nextSlideInBlock:'c1_b5_s2',diaryPrompt:"Explique a <strong>Lei de Fourier</strong> para o fluxo de calor ($$\Phi = \frac{k \cdot A \cdot \Delta T}{L}$$). Detalhe o que cada variável representa (condutividade térmica, área, variação de temperatura, espessura) e como elas influenciam a taxa de transferência de calor.",geminiCalcPrompt:`Você é a I.A. da Estação Aurora. Crie um desafio de CÁLCULO com 5 opções de resposta. O cenário: "Um painel do casco da nave, com área, espessura e condutividade térmica conhecidas, está com uma face exposta ao frio do espaço e a outra face em contato com o ambiente interno aquecido." A pergunta deve ser sobre o cálculo do fluxo de calor (potência térmica) através do painel. O feedback deve mostrar a Lei de Fourier em modo de exibição ($$\Phi = \frac{k \cdot A \cdot \Delta T}{L}$$) e o cálculo detalhado.`},
...
async function generateDynamicQuestion(slide, difficulty = 'standard') {
    ...
    const finalPrompt = basePrompt + " " + difficultyInstruction + ` **REGRA FINAL E MAIS IMPORTANTE: A sua saída deve ser APENAS um objeto JSON válido, sem nenhum texto, markdown ou formatação adicional fora do JSON. O formato DEVE ser exatamente como este exemplo, incluindo as chaves "question", "answers", "correctFeedback", "incorrectFeedback" e a propriedade "correct" (um booleano 'true' ou 'false') para cada uma das 5 respostas: ${jsonFormatExample}**`;
